# Implementation & modification of Early Drowsiness Detection
# All GUI stuffs such as tkinter, cv2.imshow, etc. are removed.
# Will be used in SwordMaster-ai.

# @author Dogdriip
# TODO: Make the structured standard(Standarization?) of output.
# NOTE: Uses standard output(stdout). Do not print anything except output texts.
...
BLINK_DETECTOR_PATH = './Trained_SVM_C=1000_gamma=0.1_for 7kNegSample.sav'
INPUT_PATH = sys.argv[1]  # the path to the input video
# The text file to write to (for blinks) # Deprecated. it doesn't matter.
OUTPUT_PATH = 'alert.txt'
RESULT = []

# this "adjust_gamma" function directly taken from : https://www.pyimagesearch.com/2015/10/05/opencv-gamma-correction/


# ANCHOR: debug function
def debug(*argv):
    if len(sys.argv) > 2:
        print(*argv)
...
def blink_detector(output_textfile, input_video):
    ...
    # ANCHOR: EMERGENCY
    # This will be used in long-time-closed alert.
    # NOTE: if ear lower than 0.21 is lasted by 50 frames, "EYES TOO LONG CLOSED".
    def EMERGENCY(ear, COUNTER):
        if ear < 0.21:
            COUNTER += 1

            if COUNTER >= 25:
                debug('EMERGENCY SITUATION (EYES TOO LONG CLOSED)')
                RESULT.append({
                    "status": "emergency",
                    "start": number_of_frames - 25,
                    "end": number_of_frames,
                })  # ANCHOR: append to res array here
                debug(COUNTER)
                COUNTER = 0
        else:
            COUNTER = 0
    ...
    def Ultimate_Blink_Check():
        ...
        # this is to split extrema in highs and lows
        THRESHOLD = 0.4*np.min(values)+0.6*np.max(values)
        N = len(values)
        ...
        # 1 means high, -1 means low for each extremum
        Updown = np.ones(len(xtrema_index))
        # this says if the extremum occurs in the upper/lower half of signal
        Updown[XtremaEAR < THRESHOLD] = -1
        ...
        if numberOfblinks > 1:
            MISSED_BLINKS = True
        if numberOfblinks == 0:
            debug(Updown, Last_Blink.duration)
            debug(values)
            debug(Derivative)
        for j in range(numberOfblinks):
            ...
            detected_blink.amplitude = 0.5 * \
                (detected_blink.startEAR-detected_blink.peakEAR) + \
                0.5*(detected_blink.endEAR-detected_blink.peakEAR)
    ...
    # ANCHOR: Blink_Tracker, main logic function.
    def Blink_Tracker(EAR, IF_Closed_Eyes, Counter4blinks, TOTAL_BLINKS, skip):
        ...
        if int(IF_Closed_Eyes) == 1:
            ...
            if Counter4blinks > 0:
                skip = False
            if Counter4blinks == 0:
                # EAR_series[6] is the EAR for the frame of interest(the middle one)
                Current_Blink.startEAR = EAR
                # reference-6 points to the frame of interest which will be the 'start' of the blink
                Current_Blink.start = reference_frame-6
            Counter4blinks += 1
                    ...
                    if Last_Blink.peakEAR < Last_Blink.startEAR and Last_Blink.peakEAR < Last_Blink.endEAR and Last_Blink.amplitude > MIN_AMPLITUDE and Last_Blink.start < Last_Blink.peak:
                        if((Last_Blink.startEAR - Last_Blink.peakEAR) > (Last_Blink.endEAR - Last_Blink.peakEAR)*0.25 and (Last_Blink.startEAR - Last_Blink.peakEAR)*0.25 < (Last_Blink.endEAR - Last_Blink.peakEAR)):  # the amplitude is balanced
                            ...
                            Last_Blink.values = signal.convolve1d(
                                Last_Blink.values, [1/3.0, 1/3.0, 1/3.0], mode='nearest')
                            ...
                            Counter4blinks = 0
                            debug("MISSED BLINKS= {}".format(
                                len(retrieved_blinks)))
                            return retrieved_blinks, int(TOTAL_BLINKS), Counter4blinks, BLINK_READY, skip
                        else:
                            skip = True
                            debug('rejected due to imbalance')
                    else:
                        skip = True
                        debug('rejected due to noise,magnitude is {}'.format(
                            Last_Blink.amplitude))
                        debug(Last_Blink.start < Last_Blink.peak)

            # if the eyes were closed for a sufficient number of frames (2 or more)
            # then this is a valid CANDIDATE for a blink
            if Counter4blinks > 1:
                # reference-7 points to the last frame that eyes were closed
                Current_Blink.end = reference_frame - 7
                ...
                if Last_Blink.duration > 15:
                    ...
                    FRAME_MARGIN_BTW_2BLINKS = 1
                # Merging two close blinks
                if (Current_Blink.start-Last_Blink.end) <= FRAME_MARGIN_BTW_2BLINKS+1:
                    debug('Merging...')
                    frames_in_between = Current_Blink.start - Last_Blink.end-1
                    debug(Current_Blink.start, Last_Blink.end, frames_in_between)
                    valuesBTW = Linear_Interpolate(
                        Last_Blink.endEAR, Current_Blink.startEAR, frames_in_between)
                    ...
                    Last_Blink.amplitude = (
                        Last_Blink.startEAR + Last_Blink.endEAR - 2 * Last_Blink.peakEAR) / 2
    ...
    # Load the Facial landmark predictor (dlib built-in?)
    debug("[INFO] loading facial landmark predictor...")
    detector = dlib.get_frontal_face_detector()

    # Load the Facial Landmark Detector
    debug("[INFO] loading Facial Landmark Detector...")
    predictor = dlib.shape_predictor(FACE_LANDMARKS_PATH)

    # Load the Blink Detector
    debug("[INFO] loading Blink Detector...")
    loaded_svm = pickle.load(open(BLINK_DETECTOR_PATH, 'rb'))
    ...
    debug("[INFO] starting video stream thread...")
    ...
    number_of_frames = 0
    while True:
        debug(f"EAR_series: {EAR_series}")

        (grabbed, frame) = stream.read()
        if not grabbed:
            debug('not grabbed')
            debug(number_of_frames)
            break
            ...
            if Q.full() and (reference_frame > 15):  # to make sure the frame of interest for the EAR vector is int the mid
                ...
                if (BLINK_READY == True):
                    ...
                    for detected_blink in retrieved_blinks:
                        debug(detected_blink.amplitude, Last_Blink.amplitude)
                        debug(detected_blink.duration, detected_blink.velocity)
                        debug('-------------------')
                        ...
                        if(detected_blink.velocity > 0):
                            with open(output_textfile, 'ab') as f_handle:
                                ...
                                np.savetxt(f_handle, [TOTAL_BLINKS, BLINK_FRAME_FREQ*100, detected_blink.amplitude,
                                                      detected_blink.duration, detected_blink.velocity], delimiter=', ', newline=' ', fmt='%.4f')
        ...
        # Does not detect any face
            ...
            if (First_frame == False):
                ...
                p1, st, err = cv2.calcOpticalFlowPyrLK(
                    old_gray, gray, leftEye, None, **lk_params)
            ...
            if Q.full() and (reference_frame > 15):
                ...
                if (BLINK_READY == True):
                    ...
                    for detected_blink in retrieved_blinks:
                        debug(detected_blink.amplitude, Last_Blink.amplitude)
                        debug(detected_blink.duration, Last_Blink.duration)
                        debug('-------------------')
                        with open(output_textfile, 'ab') as f_handle:
                            f_handle.write(b'\n')
                            np.savetxt(f_handle, [TOTAL_BLINKS, BLINK_FRAME_FREQ*100, detected_blink.amplitude,
...
#############
# ANCHOR: Main
blink_detector(OUTPUT_PATH, INPUT_PATH)
print(json.dumps(RESULT))  # Prints final result.
